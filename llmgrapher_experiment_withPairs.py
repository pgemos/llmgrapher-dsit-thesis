...
# SCRIPT DESCRIPTION
# ==============================================================================
# This script executes the Knowledge Graph generation experiment using an 
# "LLM Reranking with Triplet Context" linking strategy.
#
# Key Characteristics:
# 1. Standardization: Uses a two-step process: first retrieving candidates via 
#    VectorDB, then reranking them using an LLM.
# 2. Context Strategy: The LLM reranker is provided with "Context Triplets" 
#    (neighboring nodes and edges from the generated graph) to help disambiguate 
#    specific entities and predicates.
# 3. Batch Processing: Implements batch processing for the reranking step to 
#    optimize LLM API calls.
# 4. It produces results not just for triplets, but for pair of entities too.
# ==============================================================================
...
def standardize_graph_entities(raw_graph_df, vector_db, debug=False, debug_name="", debug_limit=5):
    if raw_graph_df.empty: return raw_graph_df

    if debug:
        print("\n" + "-"*20 + f" DEBUGGING: Entity Standardization for '{debug_name}' " + "-"*20)
...
def standardize_graph_predicates(entity_std_graph_df, vector_db, debug=False, debug_name="", debug_limit=5):
    if entity_std_graph_df.empty: return entity_std_graph_df

    if debug:
        print("\n" + "-"*20 + f" DEBUGGING: Predicate Standardization for '{debug_name}' " + "-"*20)
    ...
    for edge in tqdm(generated_edges, desc="Linking predicates", leave=False):
        ...
        final_predicate = None # Default to None if no match
        if best_match_list and isinstance(best_match_list, list):
            final_predicate = best_match_list[0]
        predicate_mapping[edge] = final_predicate

        if debug and edges_debugged < debug_limit:
            print(f"  - Raw Edge: '{edge}'")
            print(f"    - VectorDB Best Match: '{best_match_list[0] if best_match_list else 'None'}'")
            print(f"    - Final Mapped Predicate: '{final_predicate}'")
            edges_debugged += 1
...
def run_batch_experiment(nlp, id_to_name, full_freebase_df, name_to_id_map, relations_lookup, name_db, predicate_db, update_distant_supervision=UPDATE_DISTANT_SUPERVISION_ONLY_IN_CHECKPOINTS):
    ...
    for i in tqdm(range(num_samples), desc="Processing All Samples"):
        ...
        std_entity_graph_default_df = standardize_graph_entities(raw_graph_default_df, name_db, debug=DEBUG, debug_name="LLM VectorDB CR")
        fully_std_graph_default_df = standardize_graph_predicates(std_entity_graph_default_df, predicate_db, debug=DEBUG, debug_name="LLM VectorDB CR")
        
        std_entity_graph_user_df = standardize_graph_entities(raw_graph_user_df, name_db, debug=DEBUG, debug_name="LLM Inherent CR")
        fully_std_graph_user_df = standardize_graph_predicates(std_entity_graph_user_df, predicate_db, debug=DEBUG, debug_name="LLM Inherent CR")
        
        std_entity_graph_pattern_based_df = standardize_graph_entities(raw_graph_pattern_based_df, name_db, debug=DEBUG, debug_name="SVO Baseline")
        fully_std_graph_pattern_based_df = standardize_graph_predicates(std_entity_graph_pattern_based_df, predicate_db, debug=DEBUG, debug_name="SVO Baseline")
