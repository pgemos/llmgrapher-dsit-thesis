...
# SCRIPT DESCRIPTION
# ==============================================================================
# This is the DEBUG version of the "LLM Reranking with Triplet Context" experiment.
#
# Key Characteristics:
# 1. Functionality: Identical linking logic to the standard triplet-context script 
#    (VectorDB candidates + LLM Reranking using neighbor triplets).
# 2. Debugging: The 'DEBUG' flag is set to True by default.
# 3. Verbosity: Includes extensive print statements within the standardization 
#    functions to trace specific entity mappings, candidate lists, and LLM 
#    decisions for troubleshooting pipeline errors.
# ==============================================================================
...
def standardize_graph_items_simple(graph_df, vector_db, item_type='entity', debug=False, debug_name="", debug_limit=5):
    """
    ...
    if debug:
        print("\n" + "-"*20 + f" DEBUGGING: SIMPLE Standardization for '{debug_name}' ({item_type}s) " + "-"*20)
    ...
    for item in tqdm(items_to_standardize, desc=f"Linking {item_type}s (Simple)", leave=False):
        ...
        final_name = item if item_type == 'entity' else None
        if best_match_list and isinstance(best_match_list, list):
            final_name = best_match_list[0]

        item_mapping[item] = final_name
        
        if debug and items_debugged < debug_limit:
            print(f"  - Raw Item: '{item}'")
            print(f"    - VectorDB Best Match: '{best_match_list[0] if best_match_list else 'None'}'")
            print(f"    - Final Mapped Item: '{final_name}'")
            items_debugged += 1
...
def standardize_graph_items_with_batch_reranking(
    ...
    if debug:
        print("\n" + "-"*20 + f" DEBUGGING: BATCH RERANKING for '{debug_name}' ({item_type}s) " + "-"*20)
    
    name_mapping = {}
    if item_type == 'entity':
        items_to_standardize = list(pd.concat([graph_df['node_1'], graph_df['node_2']]).unique())
    else: # predicate
        ...
        try:
            ...
            for item in batch_items:
                if item in candidate_map:
                    ...
                    if debug and items_debugged < debug_limit:
                        print(f"  - Raw Item: '{item}'")
                        print(f"    - Candidates: {candidate_map[item]}")
                        print(f"    - LLM Chose: '{final_name}'")
                        items_debugged += 1
...
def orchestrate_standardization(
    ...
    if strategy == "simple":
        std_entity_df = standardize_graph_items_simple(raw_graph_df, name_db, item_type='entity', debug=debug, debug_name=debug_name)
        fully_std_df = standardize_graph_items_simple(std_entity_df, predicate_db, item_type='predicate', debug=debug, debug_name=debug_name)
    
    elif strategy == "reranking":
        std_entity_df = standardize_graph_items_with_batch_reranking(raw_graph_df, name_db, item_type='entity', batch_size=1, debug=debug, debug_name=debug_name)
        fully_std_df = standardize_graph_items_with_batch_reranking(std_entity_df, predicate_db, item_type='predicate', batch_size=1, debug=debug, debug_name=debug_name)

    elif strategy == "batch_reranking":
        std_entity_df = standardize_graph_items_with_batch_reranking(raw_graph_df, name_db, item_type='entity', batch_size=batch_size, debug=debug, debug_name=debug_name)
        fully_std_df = standardize_graph_items_with_batch_reranking(std_entity_df, predicate_db, item_type='predicate', batch_size=batch_size, debug=debug, debug_name=debug_name)
...
def run_comprehensive_evaluation(generated_df, truth_df, debug=DEBUG, debug_name=""):
    """
...
def run_batch_experiment(nlp, id_to_name, full_freebase_df, name_to_id_map, relations_lookup, name_db, predicate_db, update_distant_supervision=UPDATE_DISTANT_SUPERVISION_ONLY_IN_CHECKPOINTS):
    ...
    for i in tqdm(range(num_samples), desc="Processing All Samples"):
        ...
        fully_std_graph_default_df = orchestrate_standardization(
            ...
            debug=DEBUG, debug_name="LLM VectorDB CR"
        )
        fully_std_graph_user_df = orchestrate_standardization(
            ...
            debug=DEBUG, debug_name="LLM Inherent CR"
        )
        fully_std_graph_pattern_based_df = orchestrate_standardization(
            ...
            debug=DEBUG, debug_name="SVO Baseline"
        )
...
def load_all_dependencies(name_db_path=NAME_DB_PATH, predicate_db_path=PREDICATE_DB_PATH):
    ...
    # 2. Get all unique predicates from the entire KG
...
if __name__ == '__main__':
    ...
    output_filename = f"llmgrapher_experiment_{num_texts}_{start_time_str}_{duration_minutes}min.csv"
