...
# SCRIPT DESCRIPTION
# ==============================================================================
# This script runs the "Sentence Context" experiment against a FILTERED Knowledge Base.
#
# Key Characteristics:
# 1. Filtered KB: Uses the cleaned Freebase dataset to minimize false positive candidates.
# 2. Context: Uses the *original source sentence* for LLM reranking.
# 3. Goal: Combines the benefits of linguistic context with a cleaner search space.
# ==============================================================================
...
LINKING_STRATEGY = "batch_reranking"
...
2. The "Sentence Context" where this mention appeared.
3. A numbered list of "Candidates" from a knowledge base.

Your Thought Process for Each Item:
1.  First, I will read the "Ambiguous Mention".
2.  Next, I will carefully analyze the "Sentence Context". I will pay close attention to the other entities and the overall meaning of the sentence to understand the semantic role of the ambiguous mention. For example, if the context is "Achilles fought against Paris in the Trojan War.", I know the mention 'Paris' refers to a person from mythology, not a city.
...
- Sentence Context: "Achilles fought against Paris in the Trojan War."
- Candidates:
  1. "Paris, Texas"
  2. "Paris"
...
def standardize_graph_items_with_batch_reranking(
    ...
    """
    Standardizes items using a mini-batch reranking pipeline, using the
    original sentence from the text as context for the LLM judge.
    """
    if raw_graph_df.empty: return raw_graph_df

    print(f"Standardizing {item_type}s for graph with {len(raw_graph_df)} triplets using sentence context...")
    
    # --- Pre-computation: Find which sentence each item appears in ---
    doc = nlp(full_text)
    sentences = [sent.text for sent in doc.sents]
    item_to_sentence_map = {}

    if item_type == 'entity':
        items_to_standardize = list(pd.concat([raw_graph_df['node_1'], raw_graph_df['node_2']]).unique())
    else: # predicate
        items_to_standardize = list(raw_graph_df['edge'].unique())

    # Create a map from each item to the first sentence it appears in
    for item in items_to_standardize:
        for sentence in sentences:
            if str(item).lower() in sentence.lower():
                item_to_sentence_map[item] = sentence
                break
    ...
    for i in tqdm(range(0, len(items_to_standardize), batch_size), desc=f"Linking {item_type}s (Batch Rerank)"):
        ...
        for j, item in enumerate(batch_items):
            ...
            if not candidates:
                name_mapping[item] = item if item_type == 'entity' else None
                continue
            
            candidate_map[item] = candidates
            
            # Use the pre-computed sentence as context
            context_str = item_to_sentence_map.get(item, "No sentence context found.")
            candidate_list_str = "\\n".join([f"  {k+1}. {c}" for k, c in enumerate(candidates)])
            
            prompt_body += (f"Item {j+1}:\\n"
                             f"- Ambiguous Mention: \\\"{item}\\\"\\n"
                             f"- Sentence Context: \"{context_str.strip()}\"\\n"
                             f"- Candidates:\\n{candidate_list_str}\\n\\n")
        ...
        try:
            ...
            for item in batch_items:
                if item in candidate_map:
                    final_name = candidate_map[item][0] # Default to top-1
                    if item in answer_json and answer_json[item] in candidate_map[item]:
                        final_name = answer_json[item]
                    name_mapping[item] = final_name
        
        except Exception as e:
            print(f"Warning: Batch LLM call failed ({e}). Falling back to top-1 for this batch.")
            for item in batch_items:
                if item in candidate_map:
                    name_mapping[item] = candidate_map[item][0]

    std_df = raw_graph_df.copy()
    columns_to_standardize = ['node_1', 'node_2'] if item_type == 'entity' else ['edge']
    for col in columns_to_standardize:
        std_df[col] = std_df[col].map(name_mapping)
...
def orchestrate_standardization(
    ...
    Args:
        ...
        full_text (str): text of the sample
    ...
    """
    print(f"\\n--- Running full standardization pipeline with strategy: '{strategy}' ---")
    ...
    elif strategy in ["reranking", "batch_reranking"]:
        current_batch_size = 1 if strategy == "reranking" else batch_size
        std_entity_df = standardize_graph_items_with_batch_reranking(
            raw_graph_df, full_text, nlp, name_db, item_type='entity', batch_size=current_batch_size
        )
        fully_std_df = standardize_graph_items_with_batch_reranking(
            std_entity_df, full_text, nlp, predicate_db, item_type='predicate', batch_size=current_batch_size
        )
        
    else:
        raise ValueError(f"Unknown standardization strategy: '{strategy}'.")
...
def run_batch_experiment(nlp, id_to_name, full_freebase_df, name_to_id_map, relations_lookup, name_db, predicate_db, update_distant_supervision=UPDATE_DISTANT_SUPERVISION_ONLY_IN_CHECKPOINTS):
    ...
    for i in tqdm(range(num_samples), desc="Processing All Samples"):
        ...
        fully_std_graph_default_df = orchestrate_standardization(
            raw_graph_default_df, sample_text, nlp, name_db, predicate_db, strategy=LINKING_STRATEGY, batch_size=BATCH_RERANKING_SIZE
        )
        fully_std_graph_user_df = orchestrate_standardization(
            raw_graph_user_df, sample_text, nlp, name_db, predicate_db, strategy=LINKING_STRATEGY, batch_size=BATCH_RERANKING_SIZE
        )
        fully_std_graph_pattern_based_df = orchestrate_standardization(
            raw_graph_pattern_based_df, sample_text, nlp, name_db, predicate_db, strategy=LINKING_STRATEGY, batch_size=BATCH_RERANKING_SIZE
        )
...
if __name__ == '__main__':
    ...
    output_filename = f"llmgrapher_experiment_filtered_vectordb-llm_linking-sentences_{num_texts}_{start_time_str}_{duration_minutes}min.csv"
